...
private static final int index_WAIT_FOR_LEAVE = 12;
private void state_WAIT_FOR_LEAVE(){
  eccState = index_WAIT_FOR_LEAVE;
}
private static final int index_WAIT_FOR_LEAVE_2 = 13;
private void state_WAIT_FOR_LEAVE_2(){
  eccState = index_WAIT_FOR_LEAVE_2;
}
...
/** Services the I_PE_EVENT event. */
  public void service_I_PE_EVENT(){
    ...
    else if ((eccState == index_LEFT_CONV) && (!PE_OUT.value)) state_WAIT_FOR_LEAVE_2();
    else if ((eccState == index_WAIT_FOR_LEAVE_2) && (PE_OUT.value)) state_RELEASE_TOKE_2();
    else if ((eccState == index_SECOND_BAG) && (!PE_OUT.value)) state_WAIT_FOR_LEAVE();
    else if ((eccState == index_WAIT_FOR_LEAVE) && (PE_OUT.value)) state_RELEASE_TOKE_3();
  }
...
System.out.println("Awaiting token!");
  ...
  /** ALGORITHM DEBUG_START IN ST*/
public void alg_DEBUG_START(){
//System.out.println("In the start state: "+this);
}
}
